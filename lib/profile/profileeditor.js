    ...
    function addSingleLine(e,i) {
      ...
      for (var t=0;t<96;t++) {
        if (shouldAddTime(i, t, e.array)) {
          if (lowest === -1) { lowest = t*15; }
          select.append('<option value="' + toTimeString(t*15) + '">' + toDisplayTime(t*15) + '</option>');
        }
      }
      var selectedValue = toMinutesFromMidnight(c_profile[e.array][i].time) * 15;
